...
public class IterativeParallelism implements ListIP {
    ...
    /**
     * Constructor saves ParallelMapper
     * @param mapper to save
     */
    ...
    private <T> List<Stream<? extends T>> partitionValues(final int threads, final List<? extends T> values) {
        int actualThreads = 1;
        if (threads > 0) {
            actualThreads = Math.min(threads, values.size());
        }
        List<Stream<? extends T>> actualPartitionValues = new ArrayList<>();
        ...
        for (int i = 0; i < actualThreads; i++) {
            actualPartitionValues.add(values.subList(from, to).stream());
            from = to;
            to = to + lengthValue;
            if (mod > 0) {
                to++;
    ...
    private <T, R> R makeIP(int threads, final List<? extends T> values, Function<Stream<? extends T>, R> function,
        ...
        List<Stream<? extends T>> partitionValuesList = partitionValues(threads, values);
        if (parallelMapper !=null){
            return mergeFunction.apply(parallelMapper.map(function, partitionValuesList).stream());
        }
        List<ThreadIP<R>> threadIPs = new ArrayList<>();
        List<Thread> threadList = new ArrayList<>();
        for (Stream<? extends T> list : partitionValuesList) {
            threadIPs.add(new ThreadIP<R>() {
                @Override
                public void run() {
                    setResult(function.apply(list));
                }
            });
